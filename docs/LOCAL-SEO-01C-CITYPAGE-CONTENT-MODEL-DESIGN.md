...
| `Заборы в <городе>: расчёт, доставка и монтаж` | **VERIFIED FACT:** заголовок отвечает commercial intent, но под ним идёт длинный универсальный текст, который частично повторяет hero, service cards, gates и quiz. | **ПЕРЕРАБОТАТЬ** в короткий естественный блок `Работаем в <городе> и по <области>` без служебных labels `Зона работ`/`До выезда`/`Цена`. Содержательную часть о выборе перенести ниже карточек. |
| `citySeoText()` | **VERIFIED FACT:** четыре абзаца меняют в основном формы города/области. Полезные темы: материалы, комплектация, смета и порядок работ. `За 5 минут` и региональная «популярность» материалов не подтверждены. Тезис о постоянных бригадах подтверждён опубликованным shared trust-блоком, но их местное базирование не подтверждено. | **РАЗДЕЛИТЬ И ПЕРЕПИСАТЬ:** короткий ответ о работе в регионе наверх, shared explanatory module ниже. Сохранить проверенные операционные факты, не выдавать постоянные бригады за бригады, базирующиеся в области. |
...
3. **VERIFIED FACT:** общий trust block уже присутствует. Он прямо говорит `Свои
   ...
   Гарантию и рассрочку в новом local text повторять не нужно.
...
быстро на практические вопросы: работают ли в городе, нужно ли ждать выезда ради
первого расчёта, кто установит забор и когда станут понятны условия доставки и
итоговая цена. Читатель из другого региона может опасаться лишних затрат на дорогу.

**DESIGN RECOMMENDATION:** разделить содержимое: короткий связный текст выше
proof/cards сообщает о работе в городе и области, предварительном расчёте по
телефону, согласовании выезда и смете. Он снимает неопределённость, не обещая
одинаковую цену независимо от расстояния. После визуального выбора shared module
помогает сравнить конструкцию, комплектацию и бюджет. Объём не является KPI.
...
### Claim matrix для верхнего блока и explanatory text
...
| Свои постоянные монтажники/бригады | **USER/BUSINESS FACT:** owner-approved trust cards; тот же тезис уже в CityPage и ServicePage. | Сохранить один раз в local context, чтобы ответить, кто монтирует. Не делать вывод об их местном базировании. |
...
| Смета, комплектация, сроки, гарантийные условия | **VERIFIED FACT:** shared trust card `Договор и смета`, ServicePage и текущий CityPage. | Сохранить процесс и итоговую смету; гарантию не повторять рядом с trust block. Не обещать одинаковые сроки в каждом городе. |
...
Это тот же shared component с двумя разными по задаче content layers: короткий
связный ответ о работе в регионе выше и shared объяснение выбора ниже. Между ними
реальные проекты и существующие карточки услуг.
...
3. **Короткий local/service block:** H2 `Работаем в <городе> и по
   <области в дательном падеже>`; два коротких связных абзаца без labels `Зона
   работ`, `До выезда`, `Цена`. Указать предварительный расчёт по телефону,
   согласование выезда специалиста, доставки и монтажа, фиксацию итоговой суммы
   в смете и договоре. Один раз сказать, что устанавливают свои постоянные
   бригады. Не писать, что они физически находятся в этом городе/области, и не
   обещать отсутствие транспортных затрат. Если есть owner-confirmed nearby
   settlements, их можно ненавязчиво добавить; без них достаточно города и области.
   Hero и нижний текст уже ведут на `/tseny`, здесь третья ссылка не нужна.
4. **Project proof поднять сразу после compact блока.** Heading относится к услуге,
   ...
   `content/projects.ts`: exact-city первым, когда он есть; фактический город на
   каждой карточке. Selection deterministic, разнообразие желательно среди
   качественных фото. Это один proof block, без второго showcase.
...
7. **Shared explanatory module** после visual/service выбора и перед QuizForm:
   2–4 естественных абзаца о сравнении материалов, комплектации ворот/калитки и
   чтении стартовых цен. Один production-quality текст использует грамматические
   формы города; optional предложение о подтверждённом exact-city проекте берёт
   факт из выбранной карточки. Без exact work модуль остаётся полноценным.
   Contextual links к material pages и `/tseny` нужны только там, где помогают
   решению; обычно 2–4 на весь модуль, не quota и не список SEO anchors.
...
- compact section заменяет верхний long-text/districts вывод; нижний редакционный
  модуль общий для всех v2 routes, не 40 ручных вариантов;
- существующие service cards, price source, quiz, trust, related-city algorithm,
  project assets и schema переиспользуются;
- локализация опирается на грамматику city/oblast и project facts, а не на
...
type CityLocalContentV2 = {
  ...
  // Exceptional, owner-approved unique fact; default is the shared module.
  explanatoryOverride?: {
...
**DESIGN RECOMMENDATION:** `CityPage` использует один shared explanatory module,
а не `commercialParagraphs` в каждой city record. Он берёт `namePrepositional`,
`oblastGenitive`, общие ссылки и выбранный confirmed exact-city project. Базовый
текст полностью работает без exact project. Опциональное предложение о нём
строится из существующего `Project.title` и фактического `Project.city`; не нужно
писать новый абзац под каждый route. При реализации проверить, что title звучит
естественно в этом предложении; если нет, предложение пропустить, не изобретать
детали фотографии.

Рекомендуемый shared render contract:

```ts
renderLocalBlock(city, city.localContent?.serviceArea); // one shared prose pattern
renderExplanation(city, selectedConfirmedExactProject); // two shared paragraphs
// Exact fact is one optional sentence from Project.title; no exact = no gap.
// Only an evidence-backed explanatoryOverride replaces the default paragraphs.
```

Форма `Работаем в ${city.namePrepositional} и по ${city.oblastGenitive}` использует
уже существующие поля: для названий областей дательный падеж после `по` внешне
совпадает с хранимым родительным (`по Гродненской области`). Новый grammar field
не требуется. `CityCommercialPart` нужен только для редкого override; обычные
contextual links принадлежат shared module.

`explanatoryOverride` существует только для будущего действительно уникального
подтверждённого business fact. Поле `evidence` фиксирует источник или owner approval
внутри данных и не отображается. Пять pilot routes используют default module без
override; после pilot ещё 35 «уникальных SEO-текстов» не понадобятся.
...
- shared explanatory module, его contextual links и deterministic proof selection;
...
4. **DESIGN RECOMMENDATION / exceptional:** `explanatoryOverride` только при
   действительно уникальном подтверждённом факте с `evidence`. Pilot и обычный
   rollout обходятся без него: город и область подставляются в shared module,
   exact project fact берётся из `Project`.
...
3. `Работаем в Лиде и по Гродненской области` — точный compact copy ниже.
...
7. `Как выбрать забор для участка` — shared explanatory module ниже.
...
> **Работаем в Лиде и по Гродненской области**
>
> Чтобы узнать ориентир, не нужно сначала ждать замера. Сообщите примерную длину
> и высоту забора, материал и нужны ли ворота или калитка. По этим данным
> предварительно рассчитаем стоимость по телефону.
>
> После расчёта согласуем выезд специалиста, доставку и монтаж. Устанавливают
> забор наши постоянные бригады. Что входит в работу и сколько она будет стоить,
> зафиксируем в смете и договоре до начала монтажа.
...
**DESIGN RECOMMENDATION:** верхний блок снимает сомнение о работе за пределами
Гомеля через подтверждённую географию и понятный порядок расчёта до выезда.
Он не обещает, что поездка не влияет на смету или что бригада базируется в Лиде.
Условия выезда и доставки обсуждаются до договора, поэтому клиент может оценить
полную стоимость до решения о заказе.

### `/lida`: proposed lower explanatory copy

> **Как выбрать забор для участка**
>
> Если участок в Лиде нужно закрыть от обзора, присмотритесь к
> [профнастилу (металлопрофилю)](/zabory-iz-profnastila). У
> [евроштакетника](/zabory-iz-evroshtaketnika) можно выбрать расстояние между
> планками, а сетка-рабица оставляет периметр просматриваемым.
>
> Ворота и калитку лучше считать вместе с забором: ширина проёма, способ
> открывания и заполнение меняют комплектацию. Для сметы также нужны длина,
> высота, тип основания и особенности участка. [Стартовые цены](/tseny) помогут
> сравнить варианты, но итоговая сумма относится ко всему ограждению с доставкой
> и монтажом.

**DESIGN RECOMMENDATION:** три contextual links помогают выбрать материал и понять
роль стартовой цены. Ссылка на `/nashi-raboty` не повторяется: переход уже есть в
...
2. Shared trust section.
3. `Работаем в Глубоком и по Витебской области` — точный compact copy ниже.
...
7. `Как выбрать забор для участка` — shared explanatory module с одним
   подтверждённым exact-project предложением ниже.
...
> **Работаем в Глубоком и по Витебской области**
>
> Чтобы узнать ориентир, не нужно сначала ждать замера. Сообщите примерную длину
> и высоту забора, материал и нужны ли ворота или калитка. По этим данным
> предварительно рассчитаем стоимость по телефону.
...
**DESIGN RECOMMENDATION:** как и для Лиды, верхний блок не обещает местную базу
бригады или одинаковый тариф независимо от расстояния. Он сообщает о работе
в области, предварительном расчёте до выезда и согласовании полного объёма работ.

### `/glubokoe`: proposed lower explanatory copy

> **Как выбрать забор для участка**
>
> Если участок в Глубоком нужно закрыть от обзора, присмотритесь к
> [профнастилу (металлопрофилю)](/zabory-iz-profnastila). У
> [евроштакетника](/zabory-iz-evroshtaketnika) можно выбрать расстояние между
> планками, а сетка-рабица оставляет периметр просматриваемым. Среди работ
> выше есть подтверждённый объект в Глубоком: «Зелёный профнастил в металлической
> окантовке».
>
> Ворота и калитку лучше считать вместе с забором: ширина проёма, способ
> открывания и заполнение меняют комплектацию. Для сметы также нужны длина,
...
**DESIGN RECOMMENDATION:** одно условное предложение использует `Project.title`
подтверждённой exact-city карточки. Оно не создаёт city-specific copy field.
Три contextual links остаются общими; `/nashi-raboty` здесь не дублируется.
...
1. добавить `localContent: { version: 2 }` только этим records; `serviceArea` и
   ...
   доступен один shared explanatory module;
...
- compact section звучит как связный коммерческий текст, а не четыре labels;
  lower explanatory module работает для всех v2 routes без per-city paragraphs;
  exact sentence появляется только при confirmed exact project;
...
3. Разделить полезную функцию `citySeoText()`: короткий связный local block выше,
   а один shared explanatory module с 2–4 естественными абзацами после
   proof/service cards и перед QuizForm. Он использует формы города и optional
   confirmed exact-project sentence, но не требует текста на каждый route.
   Отдельный `districts` block объединить с service-area output.
...
7. Поднять real-project proof выше; выбирать из одного `content/projects.ts`
   примерно три карточки на city page. Для области целевой инвентарь примерно
