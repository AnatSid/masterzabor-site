...
function serviceSeoText(service: Service) {
  return [
    `${service.title} под ключ — это решение для тех, кто хочет заранее понимать бюджет, сроки и итоговый результат. Работаем по всей Беларуси. Для предварительного расчёта достаточно сообщить основные параметры объекта: примерные размеры, выбранный тип конструкции и пожелания по комплектации. Специалист уточнит детали по телефону и назовёт ориентир по стоимости. Так вы заранее понимаете бюджет и можете спокойно решить, подходит ли вам вариант. После согласования условий организуем выезд специалиста на объект для заключения договора и подготовки к работам.`,
    ...
    `Заказать ${service.title.toLowerCase()} можно для частного дома, дачи, производственной территории или нового участка. Работаем с объектами по всей Беларуси — в областных центрах, районных городах, посёлках и деревнях. Чтобы получить предварительный ориентир по стоимости, достаточно оставить номер телефона или позвонить. Уточним основные параметры и предложим понятный следующий шаг.`,
  ];
}
