...
| P1-02.2 Brand visual system foundation | in_progress | User visually approved homepage brand/benefit direction, final benefit icons and LARGE icon rendered size (`44px` mobile / `52px` `sm+`). Ready for branch commit/push, Preview verification, merge to `main` and Production smoke. |
...
- Future TODO, LOW priority / polish / technical cleanup: `Benefits icons: true-vector migration`. Current approved `public/icons/benefits/*.svg` files are temporary SVG-wrapper assets with embedded raster graphics. This is an intentional production choice to preserve the approved visual design now and is not a blocker for the next stages. A later cleanup can replace them with true vector SVG files that use real SVG primitives/paths only, with no `<image>`, base64, PNG, JPEG or WebP inside. The current production assets are the visual source of truth; the migration must preserve the approved look as close to `1:1` as possible. Do not change the design just to make vectorization easier. If the true-vector version looks worse than the current production icons, do not accept the replacement.
...
- P1-02.2 icon size selection 2026-08-24: user compared CURRENT (`32px` mobile / `40px` desktop), MEDIUM (`40px` mobile / `48px` desktop) and LARGE (`44px` mobile / `52px` desktop) on localhost and selected LARGE as final. Reference files are kept in `docs/design-concepts/benefits-icon-size-options/`, plus comparison board `docs/design-concepts/P1-02-6-benefits-icon-size-comparison.html`. Production SVG assets are unchanged; only rendered size changes.
